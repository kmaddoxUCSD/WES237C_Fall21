...
{//Write your code here
	...
	for (i = 0; i < N; i += 1) {
		...
		for (j = 0; j < N; j += 1) {
			...
			temp_real[i] += (sample_real[j] * cos_coefficients_table[i] - sample_imag[j] * sin_coefficients_table[i]);
			temp_imag[i] += (sample_real[j] * sin_coefficients_table[i] + sample_imag[j] * cos_coefficients_table[i]);
		}
	}
